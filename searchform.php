...
<form role="search" method="get" class="navbar-search d-flex align-items-center gap-2" action="<?php echo esc_url( home_url( '/' ) ); ?>">
    <label class="visually-hidden" for="<?php echo esc_attr( $search_id ); ?>"><?php esc_html_e( 'Pesquisar', 'u_correio68' ); ?></label>
    <input id="<?php echo esc_attr( $search_id ); ?>" type="search" name="s" class="form-control form-control-sm bg-light border-0" placeholder="" value="<?php echo esc_attr( get_search_query() ); ?>">
